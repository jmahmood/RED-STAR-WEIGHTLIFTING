...
//                #endif
    ...
    private func configureSession() {
        #if DEBUG
        print("ExportService: Configuring WCSession...")
        #endif
        guard WCSession.isSupported() else {
            #if DEBUG
            print("ExportService: WCSession is NOT supported on this device")
            #endif
            return
        }
        let session = WCSession.default
        #if DEBUG
        print("ExportService: Setting delegate and activating session (current state: \(session.activationState.rawValue))")
        #endif
...
extension ExportService: WCSessionDelegate {
    func session(
        ...
        print("ExportService: isCompanionAppInstalled: \(session.isCompanionAppInstalled), isReachable: \(session.isReachable)")
        #endif
        queue.async {
            if let error {
    ...
    func session(_ session: WCSession, didReceiveApplicationContext applicationContext: [String : Any]) {
        #if DEBUG
        print("ExportService: didReceiveApplicationContext: \(applicationContext.keys.sorted())")
        #endif

        // Check if this is a plan transfer via applicationContext (simulator fallback)
        if let transferType = applicationContext["transfer_type"] as? String,
           transferType == "plan_data",
           let base64Data = applicationContext["data"] as? String,
           let data = Data(base64Encoded: base64Data) {
            #if DEBUG
            print("ExportService: Received plan data via applicationContext (simulator fallback)")
            print("ExportService: Plan name: \(applicationContext["plan_name"] ?? "unknown")")
            print("ExportService: Data size: \(data.count) bytes")
            #endif

            // Create a temporary file and call the incoming handler
            ...
            do {
                ...
                #if DEBUG
                print("ExportService: Wrote plan data to temp file: \(tempURL.path)")
                #endif
                ...
                let metadata: [String: Any] = [
                    ...
                    "day_count": applicationContext["day_count"] ?? 0,
                    "sha256": applicationContext["sha256"] ?? ""
                ]

                // Call incoming handler directly with the file
                if let handler = incomingHandler {
                    // Create a simulated WCSessionFile
                    let simulatedFile = WCSessionFile()
                    #if DEBUG
                    print("ExportService: Calling incomingHandler with simulated file")
                    #endif
                    handler(simulatedFile)
                } else {
                    #if DEBUG
                    print("ExportService: No incoming handler to process plan data")
                    #endif
                    try? fileManager.removeItem(at: tempURL)
                }
            } catch {
                #if DEBUG
                print("ExportService: Failed to process plan data from applicationContext: \(error)")
                #endif
            }
        }
    }
    ...
    func session(_ session: WCSession, didReceive file: WCSessionFile) {
        #if DEBUG
        print("ExportService: ===== RECEIVED FILE FROM COMPANION =====")
        print("ExportService: File: \(file.fileURL.lastPathComponent)")
        print("ExportService: Metadata: \(String(describing: file.metadata))")
        print("ExportService: IncomingHandler exists: \(incomingHandler != nil)")
        #endif
        if let handler = incomingHandler {
            #if DEBUG
            print("ExportService: Calling incomingHandler")
            #endif
            handler(file)
        } else {
            #if DEBUG
            print("ExportService: WARNING - No incomingHandler configured!")
            #endif
        }
    }
}
